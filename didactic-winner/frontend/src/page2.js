...
const Page2 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:9090/nasa/apod?date=${date}`);
        if (!response.ok) {
        ...
        // If fetch fails, change the date to the previous day
        const previousDate = new Date(new Date(date).getTime() - 24 * 60 * 60 * 1000);
        setDate(previousDate.toISOString().split('T')[0]);
        console.log("going to yesterday 🤦‍♂️");
      }
    };
  ...
  const fetchByCount = () => {
    fetch(`http://localhost:9090/nasa/apods?count=${count}`)
      .then(response => response.json())
      .then(data => setResults2(data));
  };

  const fetchByDateRange = () => {
    fetch(`http://localhost:9090/nasa/apods?start_date=${startDate}&end_date=${endDate}`)
      .then(response => response.json())
